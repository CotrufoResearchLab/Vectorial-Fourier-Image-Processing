...
gaussian_noise_average = 0.1; %Used to add random noise to input signal. Set to zero to deactivate
gaussian_noise_variance =  0.001; %Used to add random noise to input signal. Set to zero to deactivate
Ny = 400; Nx = 400;
...
tpp = exp(-10*(KX_TF.^2 + KY_TF.^2)/NA_image^2);
tss = exp(-10*(KX_TF.^2 + KY_TF.^2)/NA_image^2);
tsp = 0*tpp;
tps = 0*tpp;
